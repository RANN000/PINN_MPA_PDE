...
def main():
    ...
    # 3. 训练模型
    print("\n3. 训练模型...")
    results_dir = Path('../results/task1_r_gpinn_mpa')
    ...
    print(f"\n训练结果已保存至\'results/task1_rgpinn_mpa\'")
...
def visualize_results(X_test, u_true, u_pred, train_loss, test_loss, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'r_gpinn_mpa_results.png', dpi=150)
    print(f"   结果已保存到: {save_dir / 'r_gpinn_mpa_results.png'}")
    plt.close()
