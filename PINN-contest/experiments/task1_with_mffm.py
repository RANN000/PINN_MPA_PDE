...
def main():
    print("=" * 70)
    print("子任务1：Helmholtz方程求解 (k=4) - mffm")
    print("=" * 70)
    ...
    # 3. 训练模型
    print("\n3. 训练模型...")
    results_dir = Path('../results/task1_mffm')
...
def visualize_results(X_test, u_true, u_pred, train_loss, test_loss, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'mffm_results.png', dpi=150)
    print(f"   结果已保存到: {save_dir / 'mffm_results.png'}")
    plt.close()
