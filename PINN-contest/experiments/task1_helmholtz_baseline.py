"""
子任务1：Helmholtz方程求解 (k=4) - Baseline实现
使用标准PINN + Adam优化器
"""
...
def main():
    print("=" * 70)
    print("子任务1：Helmholtz方程求解 (k=4) - Baseline")
    print("=" * 70)
    ...
    X_interior = X_train[~boundary_mask]  # 内部点
    X_boundary = X_train[boundary_mask]  # 边界点
    ...
    # 3. 训练模型
    print("\n3. 训练模型...")
    results_dir = Path('../results/task1_baseline')
...
def visualize_results(X_test, u_true, u_pred, train_loss, test_loss, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'baseline_results.png', dpi=150)
    print(f"   结果已保存到: {save_dir / 'baseline_results.png'}")
    plt.close()
