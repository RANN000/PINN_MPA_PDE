...
def stage1_train_forward_model(X_obs, u_obs, config):
    ...
    # MPA分析选择优化器
    print("   运行MPA分析选择优化器...")
    batch_dict = create_mpa_batch_dict(X_obs, u_obs, device)
    mpa_scores = compute_all_metrics(u_model, batch_dict, device=device, quick=True)
    ...
    print("✅ 前向模型训练完成")
    return u_model, losses, opt_name
...
def stage2_train_lambda_net(u_model, config):
    ...
    # MPA分析选择优化器
    print("   运行MPA分析选择优化器...")
    X_collocation = generate_interior_points(n_interior=1000)  # 用于MPA分析的采样点
    batch_dict = create_mpa_batch_dict(X_collocation, None, device, is_lambda_net=True)
    mpa_scores = compute_all_metrics(lambda_net, batch_dict, device=device, quick=True)
    ...
    optimizer, opt_name = select_optimizer_by_mpa(
        ...
        mpa_scores['TS'],
        mpa_scores['LSM'],
        lambda_net.parameters(),
        learning_rate=config['lr_stage2']
    )
    ...
    print("✅ 参数网络训练完成")
    return lambda_net, losses, opt_name

# mpa数据准备函数
def create_mpa_batch_dict(X_data, u_data, device, is_lambda_net=False):
    """
    为MPA分析创建batch字典
    """
    # 使用前100个点进行快速MPA分析
    n_samples = min(100, len(X_data))
    X_sample = X_data[:n_samples]

    batch_dict = {
        'domain_min': torch.tensor([-1.0, -1.0], device=device),
        'domain_max': torch.tensor([1.0, 1.0], device=device)
    }

    if not is_lambda_net and u_data is not None:
        # 对于前向模型，添加观测数据
        batch_dict['x_interior'] = torch.FloatTensor(X_sample).to(device)
        batch_dict['u_obs'] = torch.FloatTensor(u_data[:n_samples]).to(device)
    else:
        # 对于参数网络，只需要坐标
        batch_dict['x_interior'] = torch.FloatTensor(X_sample).to(device)
...
def visualize_results(u_model, lambda_net, X_obs, u_obs, u_pred, config,
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'piml_results.png', dpi=150, bbox_inches='tight')
    print(f"✅ 结果已保存到: {save_dir / 'piml_results.png'}")
    plt.close()
...
def main():
    ...
    config = {
        ...
        'results_dir': '../results/task3_piml_mpa'
    }
    ...
    # 2. 阶段1：训练前向模型
    u_model, losses_stage1, opt_name_stage1 = stage1_train_forward_model(X_obs, u_obs, config)

    # 3. 阶段2：训练参数网络
    lambda_net, losses_stage2, opt_name_stage2 = stage2_train_lambda_net(u_model, config)
    ...
    torch.save({
        ...
        'config': config,
        'mpa_info': {
            'stage1_optimizer': opt_name_stage1,
            'stage2_optimizer': opt_name_stage2
        }
    ...
    # 打印训练总结
    print(f"\n训练总结:")
    print(f"  阶段1优化器: {opt_name_stage1}")
    print(f"  阶段2优化器: {opt_name_stage2}")
...
if __name__ == "__main__":
    u_model, lambda_net, results = main()