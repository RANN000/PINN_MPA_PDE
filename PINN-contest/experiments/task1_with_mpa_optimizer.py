"""
子任务1：Helmholtz方程求解 (k=4) - 使用MPA推荐的优化器
对比baseline和MPA推荐的优化器效果
"""
...
def main():
    ...
    print(f"   内部点: {len(X_interior)}, 边界点: {len(X_boundary)}, 测试点: {len(X_test)}")
    ...
    # 3. 训练模型（使用MPA推荐的优化器）
    print("\n3. 训练模型...")
    results_dir = Path('../results/task1_mpa_recommended')
    
    import time
    start_time = time.time()
    
    # 自定义训练函数（使用MPA推荐的优化器）
    model = train_with_custom_optimizer(
        model, X_interior, X_boundary, X_test, u_test_true,
        optimizer=optimizer,
        n_epochs=2000,
        batch_size=256,
        log_every=100,
    ...
    print(f"\n训练结果已保存至\'results/task1_mpa_recommended\'")
...
def visualize_results(X_test, u_true, u_pred, train_loss, test_loss, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(save_dir / f'baseline_mpa.png', dpi=150)
    print(f"   结果已保存到: {save_dir / f'baseline_mpa.png'}")
    plt.close()
